...
# "asr-transcriptions-raw". Change the prefix if yours differs.
...
SANDBOX_SHEET_URLS = {
    # "raw": "https://docs.google.com/spreadsheets/d/.../edit?usp=sharing",
}
# ------------------------------------------------------------
...
def derive_speaker_code(tab_filename):
    ...
    stem = os.path.splitext(os.path.basename(tab_filename))[0]
    return stem.rsplit("-", 1)[1].upper() if "-" in stem else ""

def derive_prefix(wav_filename):
    """ 'RRMSAvaikiP24.wav' -> 'RRMSAvaikiP24' """
    return os.path.splitext(os.path.basename(wav_filename))[0]
